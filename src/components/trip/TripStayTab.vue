...
const {setStaies} = useTripPlanStore();
const { pickedTrips, staies: pickedStaies } = storeToRefs(useTripPlanStore());
...
const sortedStaies = computed<SearchTripWithDistance[]>(() => {
  const dayTrips = pickedTrips.value[day.value - 1];
...
const { data: staies } = useQuery({
  queryKey: ["trip", "stay", cityId],
  queryFn: () => cityStayRequest(Number(cityId)),
});
...
const setDayStay = (day: number, stay: SearchTripWithDistance) => {
  if (day === 0) {
    pickedTrips.value.forEach((_, index) => {
      setStaies(index + 1, stay);
    })
    return;
  }
  setStaies(day, stay);
...
const onSubmit = () => {
  if (pickedStaies.value.some((stay) => !stay)) {
    toast.toast({
      title: "숙소를 선택해주세요.",
...
<template>
  <Dialog>
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{ day }} 일차 숙소 선택</DialogTitle>
        ...
        <div class="w-full h-96 overflow-scroll space-y-6">
          <div v-for="stay in sortedStaies" :key="stay.tourId">
            <div class="flex flex-row items-center space-x-2 justify-between">
              ...
              />
              <div class="w-40 flex flex-row space-x-1 items-center">
              ...
              <div class="flex flex-row space-x-4">
                <Button class="w-20 h-8" variant="outline" @click="setDayStay(day, stay)"
                  >{{ day }}일차 숙소</Button
                >
                <Button class="w-20 h-8" variant="outline" @click="setDayStay(0, stay)"
                  >전체 숙소</Button
                >
              </div>
            </div>
        ...
        <div class="flex flex-col rounded-md border-gray-400 w-full min-h-12">
          <VueDraggableNext v-model="pickedTrips[index]" group="trip">
            ...
            <div
              ...
              v-if="!pickedStaies[index]"
            >
              <DialogTrigger class="w-full h-full" @click="setDay(index + 1)">
                <span
            ...
            >
              <Card className="relative flex justify-between items-center cursor-pointer">
                <X
                  ...
                  :size="14"
                  @click="setStaies(index + 1, undefined)"
                />
                <div class="flex w-12 h-12 aspect-square">
                  <img
                    :src="imageOrDefault(pickedStaies[index].backgroundImage)"
                    alt="avatar"
                    class="w-full h-full rounded-md object-cover"
                  />
                  <Skeleton class="h-full w-full rounded-xl" />
                ...
                >
                  ...
                  >
                    {{ pickedStaies[index].name }}
                  </div>
                  <div
                    className="flex flex-row justify-start space-x-1 items-center text-xs text-gray-600"
                  >
                    <span>{{ pickedStaies[index].address }}</span>
                  </div>
                </div>
              </Card>
            </div>
