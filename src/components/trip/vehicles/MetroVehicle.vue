  ...
  <TooltipProvider>
    <Tooltip>
      <div class="flex flex-row items-center w-full">
        <TramFront class="mr-5" :size="16" />
        <div
          v-if="metro"
          class="text-gray-500 dark:text-gray-400 py-4 flex flex-row justify-around w-4/5 text-xs"
        >
          <div class="flex flex-col">
            <span>소요시간: {{ convertTime(metro.spentTime) }}</span>
            <span>거리: {{ convertDistance(metro.distance) }}</span>
            <span>비용: {{ metro.fare }}원</span>
            <span v-if="metro.transferCount > 0"
              >환승 횟수: {{ metro.transferCount }}회</span
            >
          </div>
          <TooltipTrigger>
          ...
          <TooltipContent variant="outline" class="py-4">
            <div class="flex mt-3 flex-col text-xs space-y-2 w-full">
              <div
                v-for="(step, index) in metro.steps"
                :key="step.stepId"
